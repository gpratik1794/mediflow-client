...
function CampaignTester({ campaigns, purpose, phone, centreName }) {
  ...
  async function runTest() {
    if (!purpose) { setStatus('fail'); setDetail('Select a campaign purpose to test.'); return }
    if (!phone || phone.replace(/\D/g,'').length < 10) { setStatus('fail'); setDetail('Enter a valid 10-digit phone number.'); return }

    setStatus('sending'); setDetail('')

    // Use 3 dummy params (enough for any template)
    const result = await sendCampaign(
      campaigns, purpose, phone,
    ...
    <Layout title="Settings">
      <form onSubmit={handleSave} style={{ maxWidth: 680 }}>
        ...
        <Section title="Test WhatsApp">
          <div style={{ background: 'var(--bg)', borderRadius: 10, padding: '12px 16px', fontSize: 12, color: 'var(--slate)', lineHeight: 1.8 }}>
            Test any saved campaign before going live. Save settings first if you just added a campaign.
          </div>
